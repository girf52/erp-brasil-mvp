...
- **Estado em 04/08/2026:** 35 migrações, 371 testes verdes. Onda 0 fechada. Da Onda 1 está pronto tudo que não depende de contrato com terceiro: pessoa → produtos/grade → estoque → vendas → categoria financeira/DRE → financeiro (títulos) → compras → OS/serviços → crediário → esqueleto fiscal → regime tributário (dual, com Simples híbrido) → conciliação bancária
...
- **A seguir, ainda buildável:** relatórios/exportação, entrada de nota inteligente, GED leve, migração assistida, PDV (Onda 2)
